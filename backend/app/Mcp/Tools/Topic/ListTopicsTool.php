...
class ListTopicsTool extends Tool
{
     ...
     * The tool's name.
     */
    protected string $name = 'list_topics';

    /**
     * The tool's description.
     */
    protected string $description = 'List all topics (sections) within a project.';
    ...
    public function handle(Request $request, ProjectManagementService $service): Response
    {
        ...
        $result = $service->listTopics($user, $companyId, $projectId);

        if (!$result['success']) {
            return Response::error($result['error'] ?? 'Failed to list topics');
        }
